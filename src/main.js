...
import worldUvMap from "./assets/world-uv-map.jpg";
...
class RadioGlobe {
    ...
    constructor(canvas, img) {
        ...
        this.#connectionError = document.querySelector(".connection-error");
        this.#beforeButton = document.querySelector(".before-button");
    ...
    #indexOfStation = 0;
    #beforeButtonClick = () => {
        if (this.#indexOfStation - 1 < 0) {
            this.#indexOfStation = this.#stationsList.length - 1;
        }
        else {
            this.#indexOfStation -= 1;
        }

        this.#audio.src = this.#stationsList[this.#indexOfStation ].url_resolved;
        ...
        this.#audio.play()
            ...
            .finally(() => {
                this.#stationName.innerText = this.#stationsList[this.#indexOfStation].name;
                this.#playPauseBtn.src = pauseIcon;
    ...
    #nextButtonClick = () => {
        if (this.#indexOfStation + 1 === this.#stationsList.length) {
            this.#indexOfStation = 0;
        }
        else {
            this.#indexOfStation += 1;
        }
    ...
    #initStationsList = () => {
        ...
        fetch("https://de2.api.radio-browser.info/json/stations/search?country=Azerbaijan")
            .then(res => res.json())
            .then(data => {
    ...
    #playPause = () => {
        if (this.#isPlaying) {
            this.#audio.pause();
            this.#playPauseBtn.src = playIcon;
            this.#isPlaying = !this.#isPlaying;
        } else {
            this.#playPauseBtn.src = pauseIcon;
            this.#audio.play();
    ...
    render = () => {
        ...
        this.#beforeButton.addEventListener("click", this.#beforeButtonClick);
...
const globe = new RadioGlobe(canvas, worldUvMap);
globe.render();
